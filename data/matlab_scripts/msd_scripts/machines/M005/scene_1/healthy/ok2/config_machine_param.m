function conn = config_machine_param(conn)
    ...
    conn.damper_d(1) = c1_nominal .* (1 + 0.2 .* (2*rand(size(c1_nominal)) - 1));
    conn.damper_d(2) = c2_nominal .* (1 + 0.15 .* (2*rand(size(c2_nominal)) - 1));
    conn.damper_d(3) = c3_nominal .* (1 + 0.2 .* (2*rand(size(c3_nominal)) - 1));
    ...
    conn.d_wall_lin(1) = c1_wall_nominal .* (1 + 0.2 .* (2*rand(size(c1_wall_nominal)) - 1));
    conn.d_wall_lin(2) = 0;
    conn.d_wall_lin(3) = 0;
