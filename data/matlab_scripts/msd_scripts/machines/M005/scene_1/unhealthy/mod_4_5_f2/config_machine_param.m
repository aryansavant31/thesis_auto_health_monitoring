function conn = config_machine_param(conn)
    ...
    conn.damper_d(3) = c3_nominal .* (1 + 0.8 .* (2*rand(size(c3_nominal)) - 1));
    ...
    conn.k_wall_lin(2) = 0;
    conn.k_wall_lin(3) = 0;
    conn.k_wall_lin(4) = k4_wall_nominal .* (1 + 0.78 .* (2*rand(size(k4_wall_nominal)) - 1));
    conn.k_wall_lin(5) = 0;
    ...
    conn.d_wall_lin(2) = 0;
    conn.d_wall_lin(3) = 0;
    conn.d_wall_lin(4) = c4_wall_nominal .* (1 + 0.85 .* (2*rand(size(c4_wall_nominal)) - 1));
    conn.d_wall_lin(5) = 0;
