function conn = config_machine_param(conn)
    ...
    conn.mass(1) = m1_nominal .* (1 + 0.3 .* (2*rand(size(m1_nominal)) - 1));
    ...
    conn.spring_k(1) = k1_nominal .* (1 + 0.77 .* (2*rand(size(k1_nominal)) - 1));
    conn.spring_k(2) = k2_nominal .* (1 + 0.77 .* (2*rand(size(k2_nominal)) - 1));
    conn.spring_k(3) = k3_nominal .* (1 + 0.12 .* (2*rand(size(k3_nominal)) - 1));
    ...
    conn.damper_d(1) = c1_nominal .* (1 + 0.8 .* (2*rand(size(c1_nominal)) - 1));
    conn.damper_d(2) = c2_nominal .* (1 + 0.8 .* (2*rand(size(c2_nominal)) - 1));
    ...
    conn.k_wall_lin(1) = k1_wall_nominal .* (1 + 0.77 .* (2*rand(size(k1_wall_nominal)) - 1));
    conn.k_wall_lin(2) = 0;
    conn.k_wall_lin(3) = 0;
    ...
    conn.d_wall_lin(1) = c1_wall_nominal .* (1 + 0.8 .* (2*rand(size(c1_wall_nominal)) - 1));
    conn.d_wall_lin(2) = 0;
    conn.d_wall_lin(3) = 0;
