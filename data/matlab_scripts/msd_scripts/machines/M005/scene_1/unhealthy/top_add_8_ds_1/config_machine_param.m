function conn = config_machine_param(conn)
    ...
    conn.spring_k(4) = k4_nominal .* (1 + 0.1 .* (2*rand(size(k4_nominal)) - 1));
    conn.spring_k(5) = k4_nominal .* (1 + 0.1 .* (2*rand(size(k5_nominal)) - 1));
    ...
    conn.damper_d(4) = c4_nominal .* (1 + 0.3 .* (2*rand(size(c4_nominal)) - 1));
    conn.damper_d(5) = c5_nominal .* (1 + 0.3 .* (2*rand(size(c5_nominal)) - 1));
