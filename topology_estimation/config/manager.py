import os
import sys
from train_settings import TrainNRIConfig
from predict_settings import PredictNRIConfig
...
class TrainNRIConfigMain(TrainNRIConfig):
    def __init__(self):
        super().__init__()
...
def get_selected_ckpt_path():
        with open(os.path.join(TOPOLOGY_ESTIMATION_DIR, "config", "selections", "loaded_ckpt_path.txt"), "r") as f:
            ckpt_path = f.read() 
        return ckpt_path

def load_selected_config():
    log_config = TrainNRIConfigMain()

    with open(os.path.join(TOPOLOGY_ESTIMATION_DIR, "config", "selections", "loaded_config_path.txt"), "r") as f:
